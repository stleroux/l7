         ...
         <div id="addMaterial" class="col-10 offset-1 pt-2" style="display: none;">
            <div class="card card-outline card-primary mb-2">
               ...
               <div class="card-body p-3">
                  <form action="{{ route('carvings.material.store', $carving->id) }}" method="post">
                  ...
                  <tbody>
                     @foreach($carving->materials as $key => $value)
                        <tr>
                           ...
                           <td>{{$value->name}}</td>
                           <td>
                              <form action="{{ route('carvings.material.delete', $value->id) }}" method="POST" class="float-right m-0 p-0">
                                 {{csrf_field()}}
                                 {{ method_field('DELETE') }}
