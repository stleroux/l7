...
@section('pageHeader')
   <i class="{{ config('icons.invoicer-quotes') }}"></i>
	<span class="h3">Invoicer :: Quotes</span>
@endsection
...
@section('content')
	...
	@include('admin.invoicer.quotes.index.topbar')
				...
				<table class="table table-hover table-stripped table-sm" id="datatable">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<form action="{{ route('admin.invoicer.quotes.destroy', $invoice->id) }}" method="POST" 
...
<a href="{{ route('admin.invoicer.activities.create', $invoice->id) }}" class="btn btn-sm btn-outline-secondary" data-toggle="tooltip" title="New Activity">
	<i class="{{ config('icons.invoicer-newActivity') }}"></i>
</a>
@endif

									<a href="{{ route('admin.invoicer.quotes.show', $invoice->id) }}" class="btn btn-sm btn-outline-primary" data-toggle="tooltip" title="View Quote">
										<i class="{{ config('icons.view') }}"></i>
										{{-- View --}}
									</a>
									
									<a href="{{ route('admin.invoicer.quotes.edit', $invoice->id) }}" class="btn btn-sm btn-primary" data-toggle="tooltip" title="Edit Quote">
										<i class="{{ config('icons.edit') }}"></i>
									</a>

									<button type="submit" class="btn btn-sm btn-danger" data-toggle="tooltip" title="Delete Quote">
