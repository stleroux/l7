@extends('layouts.UI.app-10-2')

@section('stylesheet')
   <link rel="stylesheet" href="{{ asset('css/UI/recipes.css') }}">
@stop

@section('pageHeader')
   <i class="{{ config('icons.recipes') }} mr-1"></i>
   Recipes :: Frequently Asked Questions
@endsection

@section('breadcrumb')
   <li class="breadcrumb-item"><a href="{{ route('recipes.indexGrid', 'all') }}">Recipes</a></li>
   <li class="breadcrumb-item">Recipes FAQ</li>
@endsection

@section('rightColumn')
   @include('UI.recipes.blocks.popular')
   {{-- @include('UI.recipes.blocks.tags') --}}
@endsection

@section('topbar')
   {{-- @include('UI.recipes.faq.topbar') --}}
@endsection

@section('content')

   <div class="card mb-2 card-trans-0">
      
      <div class="card-header text-light p-2 m-0" style="background-color: #800000">
         <i class="{{ config('icons.faq') }}"></i>
         Click the question to reveal the answer.
      </div>
      
      <div class="card-body p-0 m-0">


         <div id="accordion">
            @forelse($faqs as $faq)
               <div class="card mb-2 mt-2 card-trans-8">
                  <div class="card-header p-0 m-0" style="background-color: #800000" id="heading{{ $faq->id }}">
                     <h6 class="mb-0 p-0">
                        <a href="#"
                           class="list-group-item{{--  list-group-item-action --}} p-1 m-0 card-trans-4 text-light"
                           data-toggle="collapse"
                           data-target="#collapse{{ $faq->id }}"
                           aria-expanded="true"
                           aria-controls="collapse{{ $faq->id }}">
                           {{ ucwords($faq->question) }}
                        </a>
                     </h6>
                  </div>

                  <div id="collapse{{ $faq->id }}" class="collapse" aria-labelledby="heading{{ $faq->id }}" data-parent="#accordion" style="background-color: #800000">
                     <div class="card-body p-2 m-0 card-trans-8">
                        {!! ucfirst($faq->answer) !!}
                     </div>
                  </div>
               </div>
            @empty
               <div class="col-row p-2 card-trans-4 text-light">
                  {{ config('settings.noRecordsFound') }}
               </div>
            @endforelse
         </div>

      </div>
   </div>

@endsection
